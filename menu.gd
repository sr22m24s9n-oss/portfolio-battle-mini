...
func _ready() -> void:
	...
	add_child(shade)
	var title := _label(Vector2(0, 240), "ポートフォリオミニ", 64, HORIZONTAL_ALIGNMENT_CENTER)
	title.size = Vector2(W, 90)
	var sub := _label(Vector2(0, 340), "レーン防衛リズム", 26, HORIZONTAL_ALIGNMENT_CENTER)
	...
	for opt in OPTIONS:
		...
		b.add_theme_stylebox_override("normal", _button_style(Color(0.02, 0.03, 0.05, 0.92), Color(0.25, 0.9, 1.0, 0.55)))
		b.add_theme_stylebox_override("hover", _button_style(Color(0.04, 0.08, 0.12, 0.96), Color(0.35, 0.95, 1.0, 0.85)))
